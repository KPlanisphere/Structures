...
void alta (agenda *A){
	...
	fflush(stdin);
	scanf("%s",(A+cont)->apellido);
	//gets(A[cont]->apellido);
	system("cls");
	nota();
	do{
		error=0;	
		...
		fflush(stdin);
		scanf("%s",(A+cont)->telefono);
		//gets(A[cont]->telefono);
		if (strlen((A+cont)->telefono)==10){
		system("cls");
		}else{
			error=0;
	...
	do{
		error=0;
		printf("\n\n%c D%ca: ",175,161);
		scanf("%d",&(A+cont)->fecha.dia);
		if ((A+cont)->fecha.dia<1 || (A+cont)->fecha.dia>31){
	...
	do{
		error=0;
		printf("\n\n%c Mes: ",175);
		scanf("%d",&(A+cont)->fecha.mes);
		if ((A+cont)->fecha.mes<1 || (A+cont)->fecha.mes>12){
	...
	do{
		error=0;
		printf("\n\n%c A%co: ",175,164);
		scanf("%d",&(A+cont)->fecha.anyo);
		if ((A+cont)->fecha.anyo<1941 || (A+cont)->fecha.anyo>2013){
